...
class EventsScreen extends StatelessWidget {
  ...
  Future<void> _refreshMeetups(BuildContext context) async {
    final meetupBloc = context.read<MeetupBloc>();
    meetupBloc.add(FetchMeetups());

    await meetupBloc.stream.firstWhere(
      (state) => state is MeetupLoaded || state is MeetupError,
    );
  }
          ...
          if (state is MeetupLoaded) {
            if (state.meetups.isEmpty) {
              return RefreshIndicator(
                onRefresh: () => _refreshMeetups(context),
                child: ListView(
